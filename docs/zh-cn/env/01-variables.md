...
| `SLOW_ENDPOINT_BUFFER_MS` | `10000` | 在历史成功请求的首字节耗时（TTFB）P95 上增加的缓冲，单位毫秒。样本按平台 + `endpoint_scope` 分组，有效故障转移预算为 `max(基础预算, P95 TTFB + 缓冲)`。没有历史样本的端点，以及 P95 加缓冲不超过基础预算的端点，仍使用基础预算。`0` 表示不额外增加缓冲。运行时的 `slow_endpoint_buffer_ms` 设置键优先于此环境变量。 |
